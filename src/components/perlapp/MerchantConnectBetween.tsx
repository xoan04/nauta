...
export function MerchantConnectBetween({ left, right }: MerchantConnectBetweenProps) {
  ...
  const pairMatches = requests.filter(
    (r) =>
      [r.fromMerchantId, r.toMerchantId].sort().join("|") ===
  ...
  const disabled = hasAccepted || hasPending || !activeMarketId;
  ...
  return (
    <div className="flex w-11 shrink-0 flex-col items-center justify-center self-stretch py-2">
      <button
        ...
        disabled={disabled}
        onClick={() => {
          const targetId = activeMarketId === left.id ? right.id : left.id;
          sendRequest(activeMarketId, targetId);
        }}
